...
void compute_attention_HLS(fixed_t Q[N][dk], fixed_t K[N][dk], fixed_t V[N][dv], fixed_t Output[N][dv]) {
    ...
    // #pragma HLS ARRAY_PARTITION variable=local_K dim=2 type=complete
    // #pragma HLS ARRAY_PARTITION variable=local_V dim=2 type=complete

    #pragma HLS ARRAY_PARTITION variable=local_K dim=2 factor=4 cyclic
    #pragma HLS ARRAY_PARTITION variable=local_V dim=2 factor=4 cyclic
    // BRAM binding
    ...
    #pragma HLS ARRAY_PARTITION variable=curr_Q dim=1 type=complete
    #pragma HLS ARRAY_PARTITION variable=curr_out dim=1 type=complete
    

    #pragma HLS ARRAY_PARTITION variable=att_score dim=1 factor=2 cyclic
